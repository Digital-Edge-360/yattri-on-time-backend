...
const sendReminder = async (req , res) =>{
    try {
        ...
        if(!remainder) return  res.status(404).json({ message: "No Remider Found!" });

        //If the reminder exists then finding if the user exists
        ...
        if(user.reminder <= 0) return res.status(404).json({ message: "Don't have enough reminders left" });
        ...
        // else
            ...
            user.save();
            return res.status(200).json({message:"Reminded!" , data:user.reminder});
