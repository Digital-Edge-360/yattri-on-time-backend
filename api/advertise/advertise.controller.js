...
const Add_ = (request, response) => {
    ...
    if (!title || status == undefined)
        ...
        response.status(400).json({ message: 'invalid image type' });
            ...
            advertise.save();
            response.status(201).json({ message: "data saved", data: advertise });
        }
    }
...
const Update_ = (request, response) => {
    Advertisement.findById(request.params.id).then((data) => {
        if (data == null)
    ...
    ).catch((err) => { response.status(404).json({message:"invalid id"}) });
};
...
const Remove_ = (request, response) => {
    Advertisement.findByIdAndDelete(request.params.id).then((data) => {
        if (data == null)
        response.status(400).json({message:"invalid id"});
        else {
            var filePath = __dirname + '/../../uploads/' + data.image;
